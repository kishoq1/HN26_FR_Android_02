...
class ArtistsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // THAY ĐỔI LOGIC: Khi bấm vào ca sĩ -> Mở BottomSheet danh sách bài hát thay vì phát ngay
        val adapter = PlaylistAdapter(emptyList()) { clickedArtistGroup ->
            showSongsBottomSheet(clickedArtistGroup.name, clickedArtistGroup.songs)
        }
        ...
        sharedViewModel.songs.observe(viewLifecycleOwner) { songs ->
            val groupedByArtist = songs.groupBy { it.artist }
            val artistPlaylists = groupedByArtist.map { (artistName, songsOfArtist) ->
                Playlist(name = artistName, songs = songsOfArtist.toMutableList())
            }
            adapter.updateData(artistPlaylists.sortedBy { it.name })
        }

        return view
    }

    // Hàm hiển thị danh sách bài hát chi tiết của Ca sĩ
